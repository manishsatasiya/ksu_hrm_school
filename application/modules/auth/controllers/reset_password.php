...
class Reset_password extends MX_Controller {
    ...
    public function reset($email, $token) {
        ...
        // set default message
        $this->session->set_flashdata('message', ($this->lang->line('reset_password_failed_token')));
        ...
        if (isset($time_diff) && $time_diff > Settings_model::$db_config['password_link_expires']) { // 12 hours password expired
            // link has expired
            $this->session->set_flashdata('message', ($this->lang->line('reset_password_link_expired')));
            // remove token
            ...
            if ($this->reset_password_model->update_password_and_nonce(urldecode($email), $new_password)) {
                ...
                $this->session->set_flashdata('message', ($this->lang->line('reset_password_failed_db')));
            }
        }else{
            $this->session->set_flashdata('message', ($this->lang->line('reset_password_failed_token')));
        }
