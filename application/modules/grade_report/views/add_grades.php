...
<div class="grade-rpt">
    <div id="admin">
        ...
        <div class="col-md-12">
				<?php
					print form_open('grade_report/index/section_id/asc/post/0') ."\r\n";
        		?>
...
<h4><?php echo $this->lang->line('sub_grade_p_heading'); ?>Submit Grade</h4>
	<div class="pull-right">
        <a href="<?php print base_url() ?>list_grade/callthirdmarkerpage" data-target="#myModal" data-toggle="modal" class="btn btn-primary btn-xs btn-mini">
             <?php echo $this->lang->line('sub_att_p_sea_without_marks'); ?>
        </a>
    </div>
</div>
   ...
   <!--Pagination end -->      
	...
	if(isset($grade_type_exam[$grade_type_id]))
	{
			...
			if (isset($course_classes->student)) 
			{
				...
				print form_open('grade_report/add_grades/', array('id' => 'add_grade_form'.$j, 'class' => 'add_grade_form')) ."\r\n";
			...
			?>
			<table border="1" cellpadding="5" cellspacing="0" width="1055px" id="chnstyle" class="table table-bordered no-more-tables">
				<tr>
					...
					/*else if($grade_type_exam_data["is_show_percentage"] != "Yes")
						$iscolspan = 1;*/
			...
			?>		
				<tr <?php echo $trbg;?>>
					...
					{
					?>
						<a href="<?php print base_url() ?>grade_report/view_grade_report_log/<?php echo $course_classes->section_id;?>/<?php echo $student_datas->student_uni_id; ?>/<?php echo $grade_type_id; ?>" data-target="#myModal" data-toggle="modal" style="text-decoration:underline; color:red;">
					...
					if(isset($course_classes->student_grade_data[$course_classes->section_id][$grade_type_exam_id][$student_datas->student_uni_id]["verify_editable"]))
							$verify_editable = $course_classes->student_grade_data[$course_classes->section_id][$grade_type_exam_id][$student_datas->student_uni_id]["verify_editable"];		
					...
					?>	
						<td align="center" <?php echo $bgcolortd_status_cheat;?>>
							...
							if($this->session->userdata('role_id') == '1' || (in_array("add_grades",$this->arrAction) && $editable == 0 && $verify_editable == 0))
							{
								print form_dropdown($grade_examwisestatus_combination,$arr_exam_status,$exam_status,'id="reg_name_suffix" class="input_text1 select2-container qtip_name_suffix"');
							}
							else 
							{
								echo $exam_status."&nbsp;";
						...
						?>
							...
							?>
								<td align="center">
							<?php
								print form_input(array('name' => $grade_entry_combination , 'id' => 'grade'.$k.$grade_type_exam_id, 'value'=>$exam_mark,'class' => 'admin_input','size'=>'3',''=>'','onKeyPress'=>'return stop_keypress_numaric(event)'));
							?>	
								</td>
								<td align="center">
							<?php
								print form_input(array('name' => $grade_entry_combination_2 , 'id' => 'grade2_'.$k.$grade_type_exam_id, 'value'=>$exam_mark_2,'class' => 'admin_input','size'=>'3',''=>'','onKeyPress'=>'return stop_keypress_numaric(event)'));
							?>	
								</td>
							<?php
								if(($this->session->userdata('role_id') == '1' || (in_array("add_grades",$this->arrAction) && $editable == 0 && $verify_editable == 0)) && abs($exam_mark-$exam_mark_2) >= $grade_type_exam_data["two_mark_difference"] && $exam_mark_2 !== "" && $exam_mark_2 !== NULL)
								{
								
							?>
								<td align="center" <?php echo $bgcolortd;?>>
							<?php
								print form_input(array('name' => $grade_entry_combination_3 , 'id' => 'grade3_'.$k.$grade_type_exam_id, 'value'=>$exam_mark_3,'class' => 'admin_input','size'=>'3',''=>'','onKeyPress'=>'return stop_keypress_numaric(event)'));
							?>	
								</td>
							<?php	
								}
					...
					<?php		
						...
						?>	
							<td align="center">
								...
								if($this->session->userdata('role_id') == '1' || in_array("add_grades",$this->arrAction))
								{
									print form_dropdown($grade_examwisestatus_combination,$arr_exam_status,'','id="reg_name_suffix" class="input_text1 qtip_name_suffix"');
								}
								else 
								{
									echo $exam_status."&nbsp;";
							...
							?>
								...
								if($this->session->userdata('role_id') == '1' || in_array("add_grades",$this->arrAction))
								{
									print form_input(array('name' => $grade_entry_combination , 'id' => 'grade'.$k.$grade_type_exam_id, 'value'=>'','class' => 'admin_input','size'=>'3',''=>'','onKeyPress'=>'return stop_keypress_numaric(event)'));
								}
								else
								{
									echo "&nbsp;";
								...
								if($this->session->userdata('role_id') == '1' || in_array("add_grades",$this->arrAction))
								{
									print form_input(array('name' => $grade_entry_combination_2 , 'id' => 'grade2_'.$k.$grade_type_exam_id, 'value'=>'','class' => 'admin_input','size'=>'3',''=>'','onKeyPress'=>'return stop_keypress_numaric(event)'));
								}
								else
								{
									echo "&nbsp;";
					...
					<td align="center">
						...
						if($this->session->userdata('role_id') == '1' || (in_array("add_grades",$this->arrAction) && $editable == 0 && $verify_editable == 0))
						{
							print form_dropdown($grade_status_combination,$arr_exam_status,$exam_status,'id="reg_name_suffix" class="input_text1 qtip_name_suffix"');
						}
						else 
						{
							echo $exam_status."&nbsp;";
