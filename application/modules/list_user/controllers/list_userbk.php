...
$this->template->set_partial('sidebar', 'sidebar');
	...
	public function edit_profile($user_id = 0) {
		...
		$user_data = (object) array_merge((array)$user_data,$cv_reference,array('emergency_contacts'=>$emergency_contacts));
...
$this->template->set_partial('sidebar', 'sidebar');
	...
	public function add_qualifications($user_id,$id = null){
    	...
    	$rowdata = array();
    	if($id){
    		$rowdata = $this->list_user_model->get_user_quali_certi($id,'qualification');
    	}
    	$content_data['rowdata'] = $rowdata;
    	if($this->input->post()){
