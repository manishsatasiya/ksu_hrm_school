...
class Contracted_summary_model extends CI_Model {

    public function __construct() {
        parent::__construct();
    }

	public function get_contracted_summary($limit = 0, $offset = 0, $order_by = "contractor_id", $sort_order = "asc", $search_data,$count = false) {
	...
	public function actual_total_number($contractor_id,$campus_id,$user_roll_id){
		//$this->db->select('COUNT(*) as cnt',FALSE);
        $this->db->from('users');
		$this->db->where("users.user_roll_id", $user_roll_id);
		$this->db->where("users.campus_id", $campus_id);
		$this->db->where("user_profile.contractor", $contractor_id);
		$this->db->join('user_profile','user_profile.user_id = users.user_id','left');  
		return $this->db->count_all_results();
	}
}

/* End of file contracted_summary_model.php */
